...
function crudRoutes(path: string, pick: (store: Awaited<ReturnType<typeof getStore>>) => Collection<any>) {
  ...
  adminRouter.put(`/${path}/:id`, async (req, res) => {
    const store = await getStore();
    try {
  ...
  adminRouter.delete(`/${path}/:id`, async (req, res) => {
    const store = await getStore();
...
adminRouter.get("/support-services", async (_req, res) => {
  const store = await getStore();
